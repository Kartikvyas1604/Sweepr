...
export default function SettlePage() {
  ...
  const [poolInfo, setPoolInfo] = useState<{ pool: Record<string, unknown>; memberCount: number } | null>(null);

  useEffect(() => {
    async function load() {
      try {
        const data = await api.pools.get(params.id as string);
        setLeaderboard(data.leaderboard);
        setPoolInfo(data);
      } catch {
        // Pool not found or error — keep defaults
      }
    }
    load();
  }, [params.id]);
  ...
  const totalPot = Number((poolInfo?.pool as any)?.totalStakedUsdc ?? 0);
                ...
                <Card variant="bordered">
                  ...
                  <CardContent className="flex flex-col items-center gap-6 py-8">
                    ...
                    >
                      ...
                      >
                          {winner.teamFlagUrl ? (
                            <img src={winner.teamFlagUrl} alt="" className="h-16 w-16 object-contain" />
                          ) : (
                    ...
                    >
                      ...
                      <p className="mt-1 font-body text-sm text-muted-foreground">
                        {winner.teamName} · {winner.score} points
                      </p>
                    </motion.div>
                    <motion.div
              ...
              >
                <Card>
                  <CardContent className="flex flex-col items-center gap-6 py-8">
                    ...
                    <div className="flex items-center gap-2 font-mono text-[11px] text-muted-foreground/40">
                      <motion.div
                        animate={{ rotate: 360 }}
              ...
              >
                <Card>
                  <CardContent className="flex flex-col items-center gap-6 py-8">
                    ...
                    <div className="w-full space-y-2 rounded-lg bg-muted/30 px-4 py-4">
                      <div className="flex items-center justify-between font-mono text-xs text-muted-foreground">
                        <span>Total Pot</span>
                        <span className="tabular-nums text-foreground">{totalPot} SOL</span>
                      </div>
                      <div className="flex items-center justify-between font-mono text-xs text-muted-foreground">
                        <span>Sweepr Fee (2.5%)</span>
                        <span className="tabular-nums text-foreground">
                          {(totalPot * 0.025).toFixed(4)} SOL
                        </span>
                      </div>
                    ...
                    <div className="w-full space-y-2">
                      <div className="flex items-center justify-between rounded-md bg-muted/30 px-4 py-2.5">
                        ...
                        <span className="flex items-center gap-1.5 font-mono text-xs text-accent">
                          {formatAddress(String((poolInfo?.pool as any)?.settlementTx ?? "TBD"))}
                      ...
                      <div className="flex items-center justify-between rounded-md bg-muted/30 px-4 py-2.5">
                        ...
                        <span className="font-mono text-xs text-foreground">
                          {winner?.displayName} · {winner ? formatAddress(winner.wallet) : ""}
                        </span>
                      </div>
                    </div>
